...
police_grid <- st_read("/Users/francesmurray/Desktop/PPA/Police_Grid.geojson") %>% 
...
tm_shape(police_grid_count)+ 
  tm_fill("n_car_theft", palette = "PuBuGn", style = "jenks", n= 8, title = "Number of car thefts in 2022")+ 
...
tm_shape(street_posts_count) + 
  tm_fill("count_posts", 
          ...
          style = "jenks", 
          n = 8,
          title = "number of street lamps per police grid")
...
```{r}
theft_sf <- st_as_sf(final_net). #I don't know if this line is necessary
  ...
  aggregate(., final_net, mean) %>%
   ggplot() +
     geom_sf(aes(fill=value),
             color = NA) +
     geom_sf(data = sample_n(theft_s, 1500), size = .1, col = "white", alpha = 0.5) +
     scale_fill_viridis(name = "Density") +
...
rbind(theft_KDE_sf, theft_risk_sf) %>%
  ...
  ggplot() +
    ...
    geom_sf(data = sample_n(car_theft_23, 3000), size = .5, colour = "black") +
    facet_wrap(~label, ) +
...
rbind(theft_KDE_sf, theft_risk_sf) %>%
  ...
  mutate(Pcnt_of_test_set_crimes = n_theft / sum(n_theft)) %>%
    ggplot(aes(Risk_Category,Pcnt_of_test_set_crimes)) +
      ...
      labs(title = "Risk prediction vs. Kernel density, 2024 Car Thefts",
