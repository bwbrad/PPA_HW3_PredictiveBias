...
drug_off_2022 <- crime_data %>% 
  filter(UCR_CRIME_CATEGORY == "DRUG OFFENSE" &
    grepl("2022" , OCCURRED_ON)) 
...
reg.ss.vars <- c("street_posts.nn", "lamp.isSig.dist", "n_drug_off")
  ...
  aggregate(., final_net, mean) %>%
   ggplot() +
     geom_sf(aes(fill=value),
             color = NA) +
     geom_sf(data = sample_n(theft_sf, 1500), size = .1, col = "white", alpha = 0.5) +
     scale_fill_viridis(name = "Density") +
