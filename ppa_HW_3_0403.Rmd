...
police_grid <- st_read("Data/Police_Grid/Police_Grid.shp") %>% 
  st_transform(st_crs(street_posts)) %>% 
  rename(GRID_NUMBER = GRID_NUMBE)
...
#load crime data
crime_data <- read.csv("https://raw.githubusercontent.com/bwbrad/PPA_HW3_PredictiveBias/main/crime-data_crime-data_crimestat.csv")
...
phx_villages <- st_read("Data/Villages.geojson") %>% 
  st_transform(st_crs(street_posts))

police_districts <- st_read("Data/Police_Precincts/Police_Precincts.shp") %>% 
  st_transform(st_crs(street_posts))
```
...
rbind(theft_KDE_sf, theft_risk_sf) %>%
  ...
  ggplot() +
    ...
    geom_sf(data = sample_n(street_posts, 3000), size = .1, colour = "white", alpha = .5) +
    facet_wrap(~label, ) +
...
The below graph, comparing risk prediction to kernel density, demonstrates how our model overpredicted risk in the lowest two and highest risk categories, and underpredicted it in the middle two categories.
...
We would not recommend that our model to predict the number of By assessing how well our the quality of our model prediction, it's clear that there are disparities from spatial, demographic, and categorical angles. Particularly, if our model over-predicts car thefts in areas more Black or Hispanic than Phoenix as a whole, that's a major red flag and a dangerous to send law enforcement in greater numbers to these areas. Modeling crimes based on environmental characteristics, such as streetlights, is a way to bake in "broken windows" policing into a predictive model. This will only serve to exacerbate the negative impacts policing already has on Black and Brown communities in Phoenix.
