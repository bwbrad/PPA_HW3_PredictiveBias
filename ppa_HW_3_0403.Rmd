...
tm1 <- tm_shape(street_posts_count) + 
  tm_fill("count_posts", 
          palette = "Reds", 
          style = "jenks", 
          n = 7)+ 
...
corr_plots <- final_net %>% 
  ...
  pivot_longer(cols= c(n_drug_off, street_posts.nn, count_posts),
               names_to = "metric", 
               values_to = "value") %>% 
  mutate(metric = recode(metric, "count_posts" = "Number of Street Lamps", 
                         "n_drug_off" = "Number of Drug Offenses", 
                         "street_posts.nn" = "Lamp Post NN"))
